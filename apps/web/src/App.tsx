...
function ListingCard({ listing, own, onDelete }: { listing: Listing; own?: boolean; onDelete?: (id: number) => void }) {
  return <article className="card">
    ...
    <div className="card-body"><small>{listing.isDonation ? 'DOAÇÃO' : 'À VENDA'}</small><h3>{listing.title}</h3><p>{listing.description}</p>
      ...
      {own && <button className="text-danger" onClick={() => onDelete?.(listing.id)}>Excluir</button>}</div></div>
  </article>;
}

function PublishForm({ onCreated }: { onCreated: (listing: Listing) => void }) {
  const [donation, setDonation] = useState(false); const [loading, setLoading] = useState(false); const [message, setMessage] = useState('');
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const formElement = event.currentTarget;
    setLoading(true); setMessage(''); const form = new FormData(formElement);
    const input: ListingInput = { title: String(form.get('title')), description: String(form.get('description')),
      category: String(form.get('category')), price: donation ? null : Number(form.get('price')), isDonation: donation, imageUrl: String(form.get('imageUrl')) };
    try { const result = await api.create(input); onCreated(result); formElement.reset(); setDonation(false); setMessage('Anúncio publicado!'); }
    catch (error) { setMessage(error instanceof Error ? error.message : 'Erro ao publicar.'); } finally { setLoading(false); }
  }
  return <section className="publish" id="anunciar"><div><span className="eyebrow">NOVO CICLO</span><h2>O que você quer repassar?</h2><p>Seu item parado pode ser exatamente o que outro estudante precisa.</p></div>
    <form onSubmit={submit}><label>Título<input name="title" minLength={3} required placeholder="Ex.: Calculadora científica" /></label>
      <label>Descrição<textarea name="description" minLength={10} required placeholder="Conte o estado do item e detalhes da entrega" /></label>
      <div className="form-row"><label>Categoria<select name="category">{categories.slice(1).map(c => <option key={c}>{c}</option>)}</select></label>
      <label>URL da imagem<input name="imageUrl" type="url" required placeholder="https://..." /></label></div>
      ...
      {!donation && <label>Preço (R$)<input name="price" type="number" min="0" step="0.01" required /></label>}</div>
      <button className="button" disabled={loading}>{loading ? 'Publicando…' : 'Publicar anúncio'}</button>{message && <p role="status">{message}</p>}
    </form></section>;
}

function AuthPanel({ onAuthenticated, onClose }: { onAuthenticated: (session: AuthSession) => void; onClose: () => void }) {
  const [mode, setMode] = useState<'login' | 'register'>('login'); const [loading, setLoading] = useState(false); const [message, setMessage] = useState('');
  async function submit(event: FormEvent<HTMLFormElement>) {
...
export default function App() {
  const [session, setSession] = useState<AuthSession | null>(() => api.getSession()); const [showAuth, setShowAuth] = useState(false);
  const [listings, setListings] = useState<Listing[]>([]); const [stats, setStats] = useState({ totalListings: 0, donations: 0, activeUsers: 0 });
  const [category, setCategory] = useState('Todos'); const [search, setSearch] = useState(''); const [tab, setTab] = useState<'discover' | 'mine'>('discover'); const [error, setError] = useState(''); const [actionMessage, setActionMessage] = useState('');
  const load = async () => { try { const [items, numbers] = await Promise.all([api.listings(), api.stats()]); setListings(items); setStats(numbers); } catch { setError('A API está indisponível. Inicie o backend e tente novamente.'); } };
  const refreshStats = async () => { try { setStats(await api.stats()); } catch { setActionMessage('O anúncio foi alterado, mas não foi possível atualizar as estatísticas.'); } };
  useEffect(() => { void load(); }, []);
  ...
  async function remove(id: number) {
    ...
    try {
      ...
      await refreshStats();
    } catch (error) { setActionMessage(error instanceof Error ? error.message : 'Não foi possível excluir o anúncio.'); }
  }
  function addListing(item: Listing) {
    setListings(current => [item, ...current]);
    setActionMessage('');
    void refreshStats();
  }
  function logout() { api.logout(); setSession(null); setTab('discover'); }
  return <>
    ...
    <main id="top"><section className="hero"><div className="hero-copy"><span className="eyebrow">CIRCULAR É TRANSFORMAR</span><h1>Menos descarte.<br/><em>Mais histórias.</em></h1><p>O marketplace da comunidade universitária para vender, doar e encontrar o que você precisa — perto de você.</p><div className="hero-actions"><a className="button" href="#itens">Encontrar itens</a><a className="button ghost" href="#anunciar">Quero anunciar →</a></div></div><div className="hero-art"><div className="orbit">↻</div><span className="float one">LIVROS</span><span className="float two">TECNOLOGIA</span><span className="float three">DOAÇÕES</span></div></section>
      ...
      <section className="market" id="itens"><span className="eyebrow">DESCUBRA PERTO DE VOCÊ</span><h2>Itens que merecem<br/>uma nova história.</h2>
        ...
        {error && <p className="error">{error}</p>}{actionMessage && <p className="notice" role="status">{actionMessage}</p>}<div className="grid">{visible.map(item => <ListingCard key={item.id} listing={item} own={tab === 'mine'} onDelete={remove} />)}</div>{!error && visible.length === 0 && <p className="empty">Nenhum item por aqui ainda.</p>}
      ...
      {session ? <PublishForm onCreated={addListing} /> : <section className="login-callout" id="anunciar"><span className="eyebrow">NOVO CICLO</span><h2>Entre para anunciar.</h2><p>Crie sua conta gratuita e publique itens para toda a comunidade.</p><button className="button" onClick={() => setShowAuth(true)}>Entrar ou criar conta</button></section>}
